...
function generateMixBurn1() {
    ...
    const correctAns = (5*((2*volAll*perMethane/100) + (3*volAll*perEthane/100) + (volAll*perCO/200))).toFixed(1);
    const variantsArray = [];
    ...
    const questionText = `
    Құрамында ${perMethane}% метан, ${perEthane}% этан, ${perCO}% көміртек(ІV) оксиді және ${perN}% азот болатын \
